...
def scan_scenario(scenario_dir: Path) -> dict:
    ...
    for sdir in sample_dirs:
        # ── Prefer the merged file if it exists ──────────────────────────
        # submit_hadd_validation.py produces  <scen>/<sample>.root
        # (sibling of the split directory). If present, the plotter only
        # needs to open this one file per sample → much faster than
        # iterating over every split. Fall back to the split directory
        # contents if the merged file is missing.
        merged = scenario_dir / f"{sdir.name}.root"
...
def main(argv: list[str]) -> int:
    ...
    p.add_argument("--helpers", default=None, type=Path,
                   help="Directory containing extract_structure.py "
                        "(invoked as a subprocess for the structure extraction "
                        "step). Default: same directory as --plotter. "
                        "If extract_structure.py is not present, an inline "
                        "fallback extractor is used. Note: create_sample_config.py "
                        "is NOT used by this wrapper — the YAML generation "
                        "logic is inlined here for self-containedness.")
    ...
    # sample-looking subdir which itself has at least one .root file.
    # This filters out wrapper-internal dirs (anything starting with '_'),
    # plus any orphan directory left around (e.g. log dumps) that has no
    # plottable content.
    if args.scenarios:
        scenarios = list(args.scenarios)
    else:
        scenarios = []
        for d in sorted(args.base.iterdir()):
            if not d.is_dir() or d.name.startswith("_"):
                continue
            # require at least one *.root file two levels deep
            has_root = any(d.glob("*/*.root"))
            if not has_root:
                print(f"  [skip discovery] {d.name}: no .root files inside")
                continue

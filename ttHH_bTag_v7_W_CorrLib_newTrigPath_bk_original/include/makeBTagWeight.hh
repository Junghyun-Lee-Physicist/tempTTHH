#ifndef makeBTagWeight_hh
#define makeBTagWeight_hh
...
class makeBTagWeight {
 ...
 ////      Int_t           hadFlavs[30];
       ...
       makeBTagWeight(TTree *tree=0);
       virtual ~makeBTagWeight();
...
#ifdef makeBTagWeight_cxx
makeBTagWeight::makeBTagWeight(TTree *tree) : fChain(0) 
{
    jetPt = 0;
    jetEta = 0;
    bTagScore = 0;
...
Int_t makeBTagWeight::GetEntry(Long64_t entry)
{
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t makeBTagWeight::LoadTree(Long64_t entry)
{
   if (!fChain) return -5;
...
void makeBTagWeight::setDebug(bool Debug, int nEvt){
    debug = Debug;
    numberOfEvt = nEvt;
...
TString makeBTagWeight::getInputName(){
    return ntupleName + ".root";
}
TString makeBTagWeight::getOutputName(){
//    std::cout<<"Here is the inside of [ getOutputName ]"<<std::endl;
//    TString ntupleName = "JetHT_E";

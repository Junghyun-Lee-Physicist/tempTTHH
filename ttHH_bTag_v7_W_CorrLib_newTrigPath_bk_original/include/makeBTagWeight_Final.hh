#ifndef makeBTagWeight_Final_hh
#define makeBTagWeight_Final_hh
...
////       Int_t         hadFlavs[30];
       ...
       makeBTagWeight_Final(TTree *tree=0);
       virtual ~makeBTagWeight_Final();
...
#ifdef makeBTagWeight_Final_cxx
makeBTagWeight_Final::makeBTagWeight_Final(TTree *tree) : fChain(0) 
{

}

makeBTagWeight_Final::~makeBTagWeight_Final()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t makeBTagWeight_Final::GetEntry(Long64_t entry)
{
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t makeBTagWeight_Final::LoadTree(Long64_t entry)
{
   if (!fChain) return -5;
...
void makeBTagWeight_Final::setNtupleName(TString _name){
    ntupleName = _name;
}
void makeBTagWeight_Final::setDebug(bool Debug, int nEvt){
    debug = Debug;
    numberOfEvt = nEvt;
...
TString makeBTagWeight_Final::getInputName(){
    return ntupleName + ".root";
}
TString makeBTagWeight_Final::getOutputName(){
    return ntupleName + ".root";
}

#endif // #ifdef makeBTagWeight_Final_cxx

#ifndef makeBTagWeight_reweight_hh
#define makeBTagWeight_reweight_hh
...
////       Int_t         hadFlavs[30];
       ...
       makeBTagWeight_reweight(TTree *tree=0);
       virtual ~makeBTagWeight_reweight();
...
#ifdef makeBTagWeight_reweight_cxx
makeBTagWeight_reweight::makeBTagWeight_reweight(TTree *tree) : fChain(0) 
{

}

makeBTagWeight_reweight::~makeBTagWeight_reweight()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t makeBTagWeight_reweight::GetEntry(Long64_t entry)
{
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t makeBTagWeight_reweight::LoadTree(Long64_t entry)
{
   if (!fChain) return -5;
...
void makeBTagWeight_reweight::setNtupleName(TString _name){
    ntupleName = _name;
}
void makeBTagWeight_reweight::setDebug(bool Debug, int nEvt){
    debug = Debug;
    numberOfEvt = nEvt;

    if(debug){
        std::cout<<"  [ Log ] You set the debug mode! and # of event : "<<numberOfEvt<<std::endl;
    }
}
TString makeBTagWeight_reweight::getInputName(){
    return ntupleName + ".root";
}
TString makeBTagWeight_reweight::getOutputName(){
    return ntupleName + ".root";
}

#endif // #ifdef makeBTagWeight_reweight_cxx

...
void ttHHanalyzer_base::performAnalysis(){
...
void ttHHanalyzer_base::createObjects(event * thisEvent, sysName sysType, bool up){
...
void ttHHanalyzer_base::motherReco(const TLorentzVector & dPar1p4,const TLorentzVector & dPar2p4, const float mother1mass, float & _minChi2,float & _bbMassMin1){
    float bbMass1, chi2;
...
void ttHHanalyzer_base::diMotherReco(const TLorentzVector & dPar1p4,const TLorentzVector & dPar2p4,const TLorentzVector & dPar3p4,const TLorentzVector & dPar4p4, const float mother1mass, const float  mother2mass, float & _minChi2,float & _bbMassMin1, float & _bbMassMin2){
...
void ttHHanalyzer_base::analyze(event *thisEvent){
...
void ttHHanalyzer_base::process(event* thisEvent, sysName sysType, bool up){
...
void ttHHanalyzer_base::fillHistos(event * thisEvent){
...
void ttHHanalyzer_base::writeHistos(){
    _of->file->cd();
    _histoDirs.at(0)->cd();
...
void ttHHanalyzer_base::writeTree(){
    _of->file->cd();
    _treeDirs.at(0)->cd();
    _inputTree->Write();
    //    _inputTree->Delete();
...
int main(int argc, char** argv){
    ...
    ttHHanalyzer_base analysis(cl.outputfilename, &ev, weight, true, cl.runYear, cl.DataOrMC, cl.sampleName, cl.eraName, debugVerbose);

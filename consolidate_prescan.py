...
#   genTtbarId %% 100 (17 bins, incl. expanded 61/62/71/72) : sumGenW_id_* / n_id_*
...
# genTtbarId %% 100 partition. 61/62 = tt+bbb, 71/72 = tt+4b come from the
# expanded-id reclassification done in the analyzer prescan (§5 of the analyzer
# patch): tt+nb events are MOVED OUT of 53/54/55 into 61/62/71/72, so the
# 53/54/55 bins here are the pure tt+2b remainder. Prescan files produced before
# §5 simply lack these branches and contribute 0 to them (row.get(..., 0)).
ID_KEYS = ["lt0", "0", "41", "42", "43", "44", "45",
           "51", "52", "53", "54", "55",
           "61", "62", "71", "72", "other"]
EXPANDED_KEYS = ["61", "62", "71", "72"]   # tt+nb (tt+bbb = 61,62 ; tt+4b = 71,72)
TTCAT_KEYS = ["LightFlavour", "AddCjet", "Add1Bjet_1Had",
              "Add1Bjet_2Had", "Add2Bjet", "NoTT"]

# ntuple ttCat_* is NanoAOD-genTtbarId-based and does NOT know the expanded
# split, so tt+nb events are still counted there as Add2Bjet (2b+). After §5 the
# analyzer id bins are expanded-aware, so only LF and cc map cleanly 1:1 to a
# ttCat bin -> those stay HARD checks. The b-enhanced phase space is verified by
# aggregate reconciliation instead (tt+nb has been redistributed 51-55 -> 61-72).
HARD_TTCAT_VS_ID = {
    "LightFlavour":  ["0"],
    "AddCjet":       ["41", "42", "43", "44", "45"],
}
RECON_1B_IDS  = ["51", "52"]                          # ttCat 1B1H+1B2H side
RECON_2BP_IDS = ["53", "54", "55", "61", "62", "71", "72"]  # ttCat Add2Bjet side
...
def run_crosschecks(summ: SampleSummary, rel_tol: float) -> dict:
    ...
    # (1) genTtbarId partition is complete: Σ over ALL bins == totals. Now that
    #     61/62/71/72 are included, this ALSO proves the §5 reclassification
    #     conserved every event (nothing lost or double-counted moving tt+nb out
    #     of 53/54/55). A failure here is the classic symptom of mixing pre-§5
    #     and post-§5 prescan files in one sample.
    ...
    if not checks["id_partition_count"]:
        summ.warnings.append(
            ...
            f"vs nEvents={summ.n_events:,}  (pre-§5/post-§5 file mix?)")
    if not checks["id_partition_weight"]:
        summ.warnings.append(
            f"genTtbarId weight partition mismatch: "
            f"ΣsumGenW_id={fixed_str(id_w_sum)} vs sumGenW={fixed_str(summ.sumGenW)}")

    # (2) ntuple ttCat_* partition is also complete (its 6 bins == totals).
    tt_n_sum = sum(summ.ttcat_n.values())
    tt_w_sum = sum(summ.ttcat_w.values())
    checks["ttcat_partition_count"] = (tt_n_sum == summ.n_events)
    checks["ttcat_partition_weight"] = close(tt_w_sum, summ.sumGenW)
    if not checks["ttcat_partition_count"]:
        summ.warnings.append(
    ...
    for cat, id_group in HARD_TTCAT_VS_ID.items():
        ...
        if not ok_n:
            summ.warnings.append(
                f"ttCat vs id count mismatch [{cat}]: "
    ...
    # a small diff there is an expected official-vs-ghost algorithmic migration,
    # not a job/bookkeeping error.
    return checks
...
def print_ttbar_category_table(records: list[tuple[SampleSummary, dict]]) -> None:
    """Per-sample expanded-id raw event counts — the stitching-partition input.
    Columns follow additional-b-jet multiplicity: 1b (51,52), 2b (53,54,55),
    tt+bbb (61,62), tt+4b (71,72). Only ttbar-content samples are listed."""
    cc  = lambda s: sum(s.id_n[k] for k in ("41", "42", "43", "44", "45"))
    b1  = lambda s: s.id_n["51"] + s.id_n["52"]
    b2  = lambda s: s.id_n["53"] + s.id_n["54"] + s.id_n["55"]
    bbb = lambda s: s.id_n["61"] + s.id_n["62"]
    b4  = lambda s: s.id_n["71"] + s.id_n["72"]
    rows = [s for s, _ in records if sum(s.id_n.values()) > 0]
    if not rows:
        return
    hdr = (f"{'Sample':<22}{'LF(0)':>11}{'cc(41-45)':>11}{'1b(51,52)':>12}"
           f"{'2b(53-55)':>12}{'tt+bbb(61,62)':>15}{'tt+4b(71,72)':>14}"
           f"{'lt0':>8}{'other':>8}")
    line = "─" * len(hdr)
    print("\n" + line)
    print("  EXPANDED genTtbarId %% 100 — RAW EVENT COUNTS (stitching partition)")
    print("  1b/2b = additional b-jets; tt+bbb/tt+4b = expanded (>=3 b-jets, from §5)")
    print(line)
    print(hdr)
    print(line)
    for s in rows:
        print(f"{s.sample:<22}{grouped(s.id_n['0']):>11}{grouped(cc(s)):>11}"
              f"{grouped(b1(s)):>12}{grouped(b2(s)):>12}{grouped(bbb(s)):>15}"
              f"{grouped(b4(s)):>14}{grouped(s.id_n['lt0']):>8}"
              f"{grouped(s.id_n['other']):>8}")
    print(line)


def print_expanded_reconciliation(records: list[tuple[SampleSummary, dict]]) -> None:
    """INFO: reconcile NanoAOD ttCat(2b+) against expanded id(pure 2b + tt+nb).
    A nonzero diff = tt+nb events the official categorizer had labelled 1b/LF —
    an expected, small official-vs-ghost-matching migration, not a bug."""
    rows = [s for s, _ in records
            if not s.is_data and sum(s.ttcat_n.values()) > 0]
    if not rows:
        return
    hdr = (f"{'Sample':<22}{'ttCat 2b+':>14}{'id 2b+tt+nb':>16}"
           f"{'tt+nb->1b':>10}{'tt+nb(61-72)':>15}{'tt+nb/2b+':>11}")
    line = "─" * len(hdr)
    print("\n" + line)
    print("  EXPANDED PARTITION CHECK — NanoAOD ttCat(2b+)  vs  expanded id(2b + tt+nb)")
    print("  diff>0 => some tt+nb had a NanoAOD 1b/LF label (expected, small, algorithmic)")
    print(line)
    print(hdr)
    print(line)
    for s in rows:
        tt2bp = s.ttcat_n["Add2Bjet"]
        id2bp = sum(s.id_n[k] for k in RECON_2BP_IDS)
        ttnb  = sum(s.id_n[k] for k in EXPANDED_KEYS)
        if tt2bp == 0 and id2bp == 0:
            continue
        frac = (ttnb / tt2bp) if tt2bp else 0.0
        print(f"{s.sample:<22}{grouped(tt2bp):>14}{grouped(id2bp):>16}"
              f"{grouped(id2bp - tt2bp):>10}{grouped(ttnb):>15}{frac:>11.4f}")
    print(line)
...
def write_csv(path: str, records: list[tuple[SampleSummary, dict]]) -> None:
    cols = ["sample", "type", "files_valid", "files_found",
            ...
            "n_id_2b53", "n_id_2b54", "n_id_2b55",
            "n_id_bbb61", "n_id_bbb62", "n_id_4b71", "n_id_4b72",
            "n_id_lt0", "n_id_other", "xsec_used", "eff_yield"]
    with open(path, "w", newline="") as fh:
        ...
        for s, _ in records:
            ...
            row = [
                ...
                s.id_n["61"], s.id_n["62"], s.id_n["71"], s.id_n["72"],

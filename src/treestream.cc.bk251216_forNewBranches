...
itreestream::~itreestream()
{
  std::cout<<"debug : before [ _statuscode ] and [ close() ] in ~itreestream"<<std::endl;
  _statuscode = kSUCCESS;
  close();
  std::cout<<"debug : end of ~itreestream"<<std::endl;
}
...
itreestream::close()
{
  std::cout<<"debug : before [ _statuscode ] and [ close() ] in itreestream::close()"<<std::endl;
  _statuscode = kSUCCESS;
...
//////  _tree = 0;
  _tree = nullptr;
  std::cout<<"debug : end of itreestream::close()"<<std::endl;
...
otreestream::~otreestream()
{
  std::cout<<"debug : before [ _statuscode ] and [ close() ] in ~otreestream"<<std::endl;
  _statuscode = kSUCCESS;
  close();
  std::cout<<"debug : after [ _statuscode ] and [ close() ] in ~otreestream"<<std::endl;
}
...
otreestream::close(bool closefile)
{
  ...
  std::cout<<"debug : before [ _statuscode ] in otreestream::close(bool closefile)"<<std::endl;
  _statuscode = kSUCCESS;
  ...
  std::cout<<"debug : before [ SelectedData::iterator it = selecteddata.begin(); ] in close(bool closefile)"<<std::endl;
  ...
  std::cout<<"debug : before [ if ( closefile ) ] in otreestream::close(bool closefile)"<<std::endl;
  if ( closefile )
    {
      _file->Write(0, TObject::kOverwrite); //kWriteDelete
      _file->Close(); // causes a crash in ROOT 
      delete _file;
      _file = nullptr; // added newly

      _tree = 0;
      _file = 0;
    }
  std::cout<<"debug : end of otreestream::close(bool closefile) function"<<std::endl;

...
#include "Rtypes.h"        // ClassImp 매크로 정의(header 대신 Imp)
#include "treestream.h"
#include "eventBuffer.h"
ClassImp(itreestream);       // itreestream 클래스 구현 매크로
ClassImp(otreestream);       // otreestream 클래스 구현 매크로
ClassImp(eventBuffer);       // eventBuffer 구조체 구현 매크로
    ...
    ev_(0)
{
  ...
  if (!file || file->IsZombie()) {
    std::cerr << "[Error] outputFile: cannot open output TFile: "
              << filename_ << std::endl;
    file = nullptr;
    return;
  }
...
void outputFile::close()
{
  ...
  std::cout << "debug : Begin of outputFile::close() in tnm"<<std::endl;	
...
//////  file->Close();
  ...
  std::cout << "debug : End of outputFile::close() in tnm"<<std::endl;	

//  std::cout << "==> histograms saved to file " << filename_ << std::endl;
//
//  // 1) 출력용 파일로 돌아간 뒤 히스토그램 저장
//  file->cd();
//  file->Write("", TObject::kOverwrite);
//
//  // 2) 스키밍한 TTree도 같은 출력 파일에 저장
//  if (tree) {
//    std::cout << "==> writing skimmed tree to " << filename_ << std::endl;
//    tree->Write();
//  }
//
//  // 3) 최종적으로 파일 닫기
